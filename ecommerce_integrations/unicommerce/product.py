...
# unicommerce product to DraERP item mapping
# reference: https://documentation.unicommerce.com/docs/itemtype-get.html
UNI_TO_DRAERP_ITEM_MAPPING = {
	"skuCode": "item_code",
	"name": "item_name",
...
DRAERP_TO_UNI_ITEM_MAPPING = {v: k for k, v in UNI_TO_DRAERP_ITEM_MAPPING.items()}
...
def _create_item_dict(uni_item):
	...
	for uni_field, draerp_field in UNI_TO_DRAERP_ITEM_MAPPING.items():

		value = uni_item.get(uni_field)
		if not _validate_field(draerp_field, value):
			continue

		item_dict[draerp_field] = value
...
def _get_item_group(category_code):
	"""Given unicommerce category code find the Item group in DraERP.
	...
	new_items = frappe.db.sql(
		f"""
			...
			LEFT JOIN `tabEcommerce Item` ei
				ON ei.draerp_item_code = item.item_code
				WHERE ei.draerp_item_code is NULL
...
def _build_unicommerce_item(item_code: ItemCode) -> JsonDict:
	"""Build Unicommerce item JSON using an DraERP item"""
	item = frappe.get_doc("Item", item_code)

	item_json = {}

	for draerp_field, uni_field in DRAERP_TO_UNI_ITEM_MAPPING.items():
		value = item.get(draerp_field)
...
def _handle_ecommerce_item(item_code: ItemCode) -> None:
	ecommerce_item = frappe.db.get_value(
		"Ecommerce Item", {"integration": MODULE_NAME, "draerp_item_code": item_code}
	)
