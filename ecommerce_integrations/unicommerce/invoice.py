...
def _generate_invoice(
	client: UnicommerceAPIClient, draerp_order, channel_config, warehouse_allocation=None
):
	unicommerce_so_code = draerp_order.get(ORDER_CODE_FIELD)
	...
	# TODO:  check if already generated by draerp invoice unsyced
	facility_code = draerp_order.get(FACILITY_CODE_FIELD)
...
def _fetch_and_sync_invoice(
	...
	"""Use the invoice generation response to fetch actual invoice and sync them to DraERP.
...
def create_sales_invoice(
	...
	"""Create DraERP Sales Invcoice using Unicommerce sales invoice data and related Sales order.
	...
	warehouse = settings.get_integration_to_draerp_wh_mapping(all_wh=True).get(facility_code)
...
def _get_line_items(
	...
	for item in line_items:
		item_code = ecommerce_item.get_draerp_item_code(
...
def update_cancellation_status(so_data, so) -> bool:
	...
	# partial cancels
	from ecommerce_integrations.unicommerce.cancellation_and_returns import update_draerp_order_items

	update_draerp_order_items(so_data, so)
