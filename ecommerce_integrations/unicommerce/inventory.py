...
def update_inventory_on_unicommerce(client=None, force=False):
	"""Update draERP warehouse wise inventory to Unicommerce.
	...
	warehouses = settings.get_draerp_warehouses()
	wh_to_facility_map = settings.get_draerp_to_integration_wh_mapping()
	...
	for warehouse in warehouses:
		...
		if is_group_warehouse:
			draerp_inventory = get_inventory_levels_of_group_warehouse(
				warehouse=warehouse, integration=MODULE_NAME
			)
		else:
			draerp_inventory = get_inventory_levels(warehouses=(warehouse,), integration=MODULE_NAME)

		if not draerp_inventory:
			continue

		draerp_inventory = draerp_inventory[:MAX_INVENTORY_UPDATE_IN_REQUEST]

		# TODO: consider reserved qty on both platforms.
		inventory_map = {d.integration_item_code: cint(d.actual_qty) for d in draerp_inventory}
		...
		if status:
			# update success_map
			sku_to_ecom_item_map = {d.integration_item_code: d.ecom_item for d in draerp_inventory}
