...
def fully_cancel_orders(unicommerce_order_codes: List[str]) -> None:
	""" Perform "cancel" action on DraERP sales orders which are fully cancelled in Unicommerce."""
...
def update_partially_cancelled_orders(orders, client: UnicommerceAPIClient) -> None:
	...
	for order in recently_changed_orders:
		...
		if not so_data:
			continue
		update_draerp_order_items(so_data)
...
def update_draerp_order_items(so_data, so=None):
	"""Update cancelled items in DraERP order."""
...
def _delete_cancelled_items(draerp_items, cancelled_items):
	items = [
		d.as_dict() for d in draerp_items if d.get(ORDER_ITEM_CODE_FIELD) not in cancelled_items
	]
