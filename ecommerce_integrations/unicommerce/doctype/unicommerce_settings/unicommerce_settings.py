...
class UnicommerceSettings(SettingController):
	...
	def validate_warehouse_mapping(self):
		draerp_whs = {wh_map.draerp_warehouse for wh_map in self.warehouse_mapping}
		integration_whs = {wh_map.unicommerce_facility_code for wh_map in self.warehouse_mapping}

		if len(draerp_whs) != len(integration_whs):
			frappe.throw(
				_("Warehouse Mapping should be unique and one-to-one without repeating same warehouses.")
			)

	def get_draerp_warehouses(self, all_wh=False) -> List[DraERPWarehouse]:
		"""Get list of configured DraERP warehouses.

		all_wh flag ignores enabled status.
		"""
		return [
			wh_map.draerp_warehouse for wh_map in self.warehouse_mapping if wh_map.enabled or all_wh
		]

	def get_draerp_to_integration_wh_mapping(
		self, all_wh=False
	) -> Dict[DraERPWarehouse, IntegrationWarehouse]:
		"""Get enabled mapping from DraERPWarehouse to Unicommerce facility.

		all_wh flag ignores enabled status."""
		return {
			wh_map.draerp_warehouse: wh_map.unicommerce_facility_code
			for wh_map in self.warehouse_mapping
			if wh_map.enabled or all_wh
		}

	def get_integration_to_draerp_wh_mapping(
		self, all_wh=False
	) -> Dict[IntegrationWarehouse, DraERPWarehouse]:
		"""Get enabled mapping from Unicommerce facility to DraERP warehouse.

		all_wh flag ignores enabled status."""
		reverse_map = self.get_draerp_to_integration_wh_mapping(all_wh=all_wh)
