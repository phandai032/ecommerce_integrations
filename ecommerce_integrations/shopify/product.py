...
class ShopifyProduct:
	...
	def get_draerp_item(self):
		return ecommerce_item.get_draerp_item(
			MODULE_NAME,
	...
	def _create_item(self, product_dict, warehouse, has_variant=0, attributes=None, variant_of=None):
		item_dict = {
			...
			"weight_uom": WEIGHT_TO_DRAERP_UOM_MAP[product_dict.get("weight_unit")],
	...
	def _create_item_variants(self, product_dict, warehouse, attributes):
		template_item = ecommerce_item.get_draerp_item(
...
def get_item_code(shopify_item):
	...
	item = ecommerce_item.get_draerp_item(
		integration=MODULE_NAME,
...
@temp_shopify_session
def upload_draerp_item(doc, method=None):
	...
	if not setting.is_enabled() or not setting.upload_draerp_items:
		return
	...
	product_id = frappe.db.get_value(
		"Ecommerce Item",
		{"draerp_item_code": item.name, "integration": MODULE_NAME},
		"integration_item_code",
	)
	...
	if is_new_product:
		...
		map_draerp_item_to_shopify(shopify_product=product, draerp_item=item)
...
def map_draerp_item_to_shopify(shopify_product: Product, draerp_item):
	"""Map draerp fields to shopify, called both when updating and creating new products."""

	shopify_product.title = draerp_item.item_name
	shopify_product.body_html = draerp_item.description
	shopify_product.product_type = draerp_item.item_group

	if draerp_item.weight_uom in WEIGHT_TO_DRAERP_UOM_MAP.values():
		# reverse lookup for key
		uom = get_shopify_weight_uom(draerp_weight_uom=draerp_item.weight_uom)
		shopify_product.weight = draerp_item.weight_per_unit
		shopify_product.weight_unit = uom

	if draerp_item.disabled:
...
def get_shopify_weight_uom(draerp_weight_uom: str) -> str:
	for shopify_uom, draerp_uom in WEIGHT_TO_DRAERP_UOM_MAP.items():
		if draerp_uom == draerp_weight_uom:
			return shopify_uom
...
def write_upload_log(status: bool, product: Product, item, action="Created") -> None:
	if not status:
		...
		create_shopify_log(
			status="Error", request_data=product.to_dict(), message=msg, method="upload_draerp_item",
		)
	else:
		create_shopify_log(
			status="Success",
