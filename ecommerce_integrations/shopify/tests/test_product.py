...
class TestProduct(TestCase):
	def test_sync_single_product(self):
		...
		item = product.get_draerp_item()

		self.assertEqual(frappe.get_last_doc("Item").item_code, item.item_code)

		ecommerce_item_exists = frappe.db.exists("Ecommerce Item", {"draerp_item_code": item.name})
	...
	def test_sync_product_with_variants(self):
		...
		item = product.get_draerp_item()  # should return template item
		...
		ecom_variants = frappe.db.get_list(
			"Ecommerce Item", filters={"variant_of": item.name}, fields="draerp_item_code"
		)

		created_variants = [v.name for v in variants]
		created_ecom_variants = [e.draerp_item_code for e in ecom_variants]
