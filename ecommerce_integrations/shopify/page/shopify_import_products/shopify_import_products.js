...
shopify.ProductImporter = class {
                        ...
                        <div class="card border-0 shadow-sm p-3 mb-3 rounded-sm" style="background-color: var(--card-bg)">
                            ...
                            <div id="shopify-sync-info">
                                ...
                                <div class="product-count py-3 d-flex justify-content-stretch">
                                    ...
                                    <div class="text-center p-3 mx-2 rounded w-100" style="background-color: var(--bg-color)">
                                        <h2 id="count-products-draerp">-</h2>
                                        <p class="text-muted m-0">in DraERP</p>
                                    </div>
                                    <div class="text-center p-3 mx-2 rounded w-100" style="background-color: var(--bg-color)">
	...
	async fetchProductCount() {
		...
		try {
			const { message: { draerpCount, shopifyCount, syncedCount } } = await frappe.call({ method: 'ecommerce_integrations.shopify.page.shopify_import_products.shopify_import_products.get_product_count' });

			this.wrapper.find('#count-products-shopify').text(shopifyCount);
			this.wrapper.find('#count-products-draerp').text(draerpCount);
	...
	logSync() {
		...
		const _draerpCounter = $('#count-products-draerp');
		...
		frappe.realtime.on('shopify.key.sync.all.products', ({ message, synced, done, error }) => {
			...
			if (synced) this.updateSyncedCount(_syncedCounter, _draerpCounter);
	...
	updateSyncedCount(_syncedCounter, _draerpCounter) {
		let _synced = parseFloat(_syncedCounter.text());
		let _draerp = parseFloat(_draerpCounter.text());

		_syncedCounter.text(_synced + 1);
		_draerpCounter.text(_draerp + 1);
