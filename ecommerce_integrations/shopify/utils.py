...
def _get_items_to_migrate() -> List[_dict]:
	...
	old_data = frappe.db.sql(
		"""SELECT item.name as draerp_item_code, shopify_product_id, shopify_variant_id, item.variant_of, item.has_variants
			FROM tabItem item
			LEFT JOIN `tabEcommerce Item` ei on ei.draerp_item_code = item.name
			WHERE ei.draerp_item_code IS NULL AND shopify_product_id IS NOT NULL""",
		as_dict=True,
	)
...
def _create_ecommerce_items(items: List[_dict]) -> None:
	for item in items:
		if not all((item.draerp_item_code, item.shopify_product_id, item.shopify_variant_id)):
			continue
		...
		ecommerce_item = frappe.get_doc(
			{
				...
				"draerp_item_code": item.draerp_item_code,
