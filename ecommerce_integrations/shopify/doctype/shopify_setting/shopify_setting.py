...
class ShopifySetting(SettingController):
	...
	def _validate_warehouse_links(self):
		for wh_map in self.shopify_warehouse_mapping:
			if not wh_map.draerp_warehouse:
				frappe.throw(_("DraERP warehouse required in warehouse map table."))
	...
	def update_location_table(self):
		...
		map it with correct DraERP warehouse."""
	...
	def get_draerp_warehouses(self) -> List[DraERPWarehouse]:
		return [wh_map.draerp_warehouse for wh_map in self.shopify_warehouse_mapping]

	def get_draerp_to_integration_wh_mapping(self) -> Dict[DraERPWarehouse, IntegrationWarehouse]:
		return {
			wh_map.draerp_warehouse: wh_map.shopify_location_id
			for wh_map in self.shopify_warehouse_mapping
		}

	def get_integration_to_draerp_wh_mapping(self) -> Dict[IntegrationWarehouse, DraERPWarehouse]:
		return {
			wh_map.shopify_location_id: wh_map.draerp_warehouse
			for wh_map in self.shopify_warehouse_mapping
		}
