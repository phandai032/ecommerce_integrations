...
def validate_details(invoice, center, error_logs):
	...
	cost_center = center.get("draerp_cost_center")
	if not cost_center:
		cost_center_err_msg = _("Center {0} is not linked to any DraERP Cost Center.").format(
			frappe.bold(center.get("center_name"))
		)
		make_error_log_msg(invoice, cost_center_err_msg, error_logs)

	warehouse = center.get("draerp_warehouse")
	if not warehouse:
		err_msg = _("Center {0} is not linked to any DraERP Warehouse.").format(
			frappe.bold(center.get("center_name"))
		)
		make_error_log_msg(invoice, err_msg, error_logs)
...
def process_sales_line_items(invoice, cost_center, center):
	...
	for line_item in invoice:
		...
		if not sold_by:
			msg = _("Employee {} not found in DraERP.").format(frappe.bold(line_item["employee"]["name"]))
