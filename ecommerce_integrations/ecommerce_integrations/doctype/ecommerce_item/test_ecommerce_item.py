...
class TestEcommerceItem(unittest.TestCase):
	...
	def test_get_draerp_item(self):
		self._create_doc()
		a = ecommerce_item.get_draerp_item("shopify", "T-SHIRT")
		b = frappe.get_doc("Item", "_Test Item")
		self.assertEqual(a.name, b.name)
		self.assertEqual(a.item_code, b.item_code)

		unknown = ecommerce_item.get_draerp_item("shopify", "Unknown item")
		self.assertEqual(unknown, None)

	def test_get_draerp_item_variant(self):
		self._create_variant_doc()
		a = ecommerce_item.get_draerp_item("shopify", "T-SHIRT", "T-SHIRT-RED")
		b = frappe.get_doc("Item", "_Test Item 2")
		self.assertEqual(a.name, b.name)
		self.assertEqual(a.item_code, b.item_code)

	def test_get_draerp_item_sku(self):
		self._create_doc_with_sku()
		a = ecommerce_item.get_draerp_item("shopify", "T-SHIRT", sku="TEST_ITEM_1")
		b = frappe.get_doc("Item", "_Test Item")
		self.assertEqual(a.name, b.name)
		self.assertEqual(a.item_code, b.item_code)
