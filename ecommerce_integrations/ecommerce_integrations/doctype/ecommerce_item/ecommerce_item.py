...
class EcommerceItem(Document):
	draerp_item_code: str  # item_code in DraERP
	...
	variant_of: str  # template id of DraERP item
	sku: str  # SKU
	...
	def check_unique_constraints(self) -> None:
		...
		unique_integration_item_code = {
			...
			"draerp_item_code": self.draerp_item_code,
...
def get_draerp_item_code(
	...
	return frappe.db.get_value("Ecommerce Item", filters, fieldname="draerp_item_code")
...
def get_draerp_item(
	...
	"""Get DraERP item for specified ecommerce_item.
	...
	if sku:
		item_code = frappe.db.get_value(
			"Ecommerce Item", {"sku": sku, "integration": integration}, fieldname="draerp_item_code"
		)
	if not item_code:
		item_code = get_draerp_item_code(
...
def create_ecommerce_item(
	...
	"""Create Item in draerp and link it with Ecommerce item doctype.
	...
	ecommerce_item = frappe.get_doc(
		{
			...
			"draerp_item_code": new_item.name,
